...
class AppXPC: NSObject, NSXPCListenerDelegate {
    ...
    init(exportedObject: HelperProtocol, onConnect: @escaping () -> Void, onClose: @escaping () -> Void) {
        ...
        super.init()
        XPCSecurity.applyListenerClientRequirement(to: listener)
    ...
    func listener(_: NSXPCListener, shouldAcceptNewConnection connection: NSXPCConnection) -> Bool {
        guard XPCSecurity.isAuthorizedAppConnection(connection) else {
            NSLog("Rejected unauthorized XPC connection")
            return false
        }
