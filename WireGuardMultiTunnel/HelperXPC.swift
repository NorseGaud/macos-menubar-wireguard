...
class HelperXPC {
    ...
    func helperConnection() -> NSXPCConnection? {
        ...
        XPCSecurity.applyHelperRequirement(to: xpcHelperConnection!)
        xpcHelperConnection?.resume()

        if !XPCSecurity.isAuthorizedHelperConnection(xpcHelperConnection!) {
            NSLog("Rejected helper with invalid code signature")
            xpcHelperConnection?.invalidate()
