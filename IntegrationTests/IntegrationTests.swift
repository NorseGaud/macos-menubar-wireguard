...
class IntegrationTests: XCTestCase {
    override func setUp() {
        ...
        let brewPrefix = ProcessInfo.processInfo.environment["INTEGRATION_BREW_PREFIX"] ?? defaultBrewPrefix
        let wireguardConfigDir = "\(brewPrefix)\(defaultConfigPath)"
        let requiredTestConfigFiles = [testConfigFile, testConfigFileInvalid, testConfigFileUsrLocal]
            .map { "\(wireguardConfigDir)/\($0).conf" }
    ...
    func testGetTunnels() throws {
        let bundle = Bundle(for: type(of: self).self)

        let testConfig = try String(contentsOfFile: XCTUnwrap(bundle.path(forResource: testConfigFile, ofType: "conf")))
    ...
    func testPreferences() {
        ...
        let wireguardMultiTunnelPreferencesWindow = app.windows["WireGuardMultiTunnel Preferences"]
        wireguardMultiTunnelPreferencesWindow.checkBoxes["Show details on all tunnels"].click()
        wireguardMultiTunnelPreferencesWindow.buttons[XCUIIdentifierCloseWindow].click()
